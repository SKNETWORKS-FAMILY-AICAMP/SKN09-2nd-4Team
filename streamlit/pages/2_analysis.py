...
if st.sidebar.button('확인'):
    ...
    col3, col4 = st.columns([4, 5])  # 두 열로 분할
    with col3:
    ...
    with col4:
        ...
        churn_credit_score_chart = alt.Chart(df_selection).mark_point().encode(
            x=alt.X('credit_score:Q', scale=alt.Scale(domain=[credit_score_min, credit_score_max])),
            y='balance:Q',
            color=alt.Color('churn:N', scale=alt.Scale(range=random.sample(color_palette, len(df_selection['churn'].unique()))))
        )        
    
        st.altair_chart(churn_credit_score_chart, use_container_width=True)
    ...
    with col5:
        ...
        st.altair_chart(churn_tenure_chart, use_container_width=True)
    ...
    with col6:
        ...
        st.altair_chart(churn_products_number_chart, use_container_width=True)

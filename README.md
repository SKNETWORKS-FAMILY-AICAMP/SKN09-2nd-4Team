...
- ML 모델 선택 이유: 해당 ABC 은행의 비선형 데이터셋에 최적화된 ensemble 모델을 목표로 11개의 ML 모델 중 분류성능이 뛰어난 4개의 상위 모델을 선별하였음. 조합 방식은 stacking이 제일 적절하다고 판단함.
...
- 조이현: EDA 과정에서 범주형 데이터 컬럼 간의 상관계수 편차가 큰 것을 확인했다. 따라서, 각각의 최적화된 머신러닝을 튜닝하는 것까지 시도해보고 싶었지만, 앙상블 모델의 스태킹에 있어서 예외적인 설계를 새로이 발견하여 전체 데이터셋에 적용하는 시행착오를 겪으면서 시간적 여건이 되지 않아 아쉬울 따름이다.
